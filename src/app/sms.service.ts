    ...
    sendGroupSMS(
        ...
        if (type === 'BeforeVisit') {
            url += `?body=${encodeURIComponent(
                SMSTextParking.replace('$NAME$님 ', '').replace('$ID_URL$', '')
            )}`;
        } else if (type === 'Confirm') {
            url += `?body=${encodeURIComponent(
                SMStextForConfirm.replace('$NAME$님 ', '').replace(
                    '$ID_URL$',
                    ''
                )
            )}`;
            ...
            url += `?body=${encodeURIComponent(
                SMStextForAccount.replace('$NAME$님 ', '').replace(
                    '- 예약금: $MONEY$원',
                    ''
                )
            )}`;
...
//ex: http://localhost:4200/#/booking-search?id=231207144600

const SMSTextParking = `$NAME$님 안녕하세요? 능운대펜션입니다. 방문일이 다가와 연락드립니다.
필요한 경우, 아래 링크에 접속하시어 <차량등록>, <식사예약> 등 사전 정보를 입력해주시기 바랍니다.
https://hwayanghotel.github.io/#/booking-search?$ID_URL$
감사합니다.`;

const SMStextForAccount = `$NAME$님 안녕하세요. 능운대펜션입니다. 입금 계좌 정보를 안내드립니다.
입금 순서대로 예약이 완료되며, 확인 후 연락드리겠습니다. 감사합니다.
- 입금계좌: 농협 352-0370-5919-43 (예금주: 정경미)
- 예약금: $MONEY$원`;

const SMStextForConfirm = `$NAME$님 안녕하세요. 능운대펜션입니다. 예약 확정되어 안내드립니다.
필요한 경우, 아래 링크에 접속하시어 <차량등록>, <식사예약> 등 사전 정보를 입력해주시기 바랍니다.
https://hwayanghotel.github.io/#/booking-search?$ID_URL$
감사합니다.`;
...
https://hwayanghotel.github.io/#/booking-search
감사합니다.`;
